...
function getUnitNumber() {
  ...
  const match = window.location.pathname.match(/vf1-u(\d{2})/i);
...
function makeSubmitUrl(unit) {
  ...
  const url = new URL("https://docs.google.com/forms/d/e/1FAIpQLScOOy3NTX1aIuHk_tfVG-LGsOHFsyJdU0PLGlSFizR2CFWK0g/viewform");
...
function hydrate(unit) {
  ...
  document.querySelector("#unit-code").textContent =
    `VOCABULARY FOUNDATION 1 · ${unit.code}`;
  ...
  document.querySelector("#unit-summary").textContent =
    `${unit.entries.length} mục từ học thuật nền tảng: học nghĩa, cách dùng và phát âm chuẩn.`;
...
const unit = window.VF1_UNITS?.[unitNumber];
if (!unit) {

(function () {
  ...
  var notebookOnly = Boolean(correctionMatch || academicMatch || grammarMatch);
  var submissionKind = correctionMatch ? "correction" : (academicMatch ? "academic" : (grammarMatch ? "grammar" : "vocabulary"));
  var program = correctionMatch
    ...
    : (academicMatch
      ...
      : (grammarMatch ? Object.freeze({ name: "Grammar Foundation", units: 23 }) : (codeMatch ? PROGRAMS[codeMatch[1]] : null)));
  var matchedUnit = correctionMatch
    ? Number(correctionMatch[2])
    : (academicMatch ? Number(academicMatch[1]) : (grammarMatch ? grammarMatch[1] : (codeMatch ? Number(codeMatch[2]) : 0)));
  var validVocabulary = Boolean(codeMatch && program && Number(matchedUnit) <= program.units);
  var assignmentCode = (notebookOnly || validVocabulary) ? requestedCode : "";

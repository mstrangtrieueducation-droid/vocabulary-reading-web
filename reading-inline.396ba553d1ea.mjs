...
export function planInline(label, items) {
  if (!label || !items.length || items.some(q => q.kind !== 'text' || q.contextTable || q.contextImage || q.sketchMap || q.extensionPrompt || q.wordParts)) return null;
  const parts = label.split(/\n\n+/);
  const byNumber = new Map();
  for (const q of items) {
    const match = q.prompt.match(/^\s*(\d+)[.)]\s/);
    if (!match || byNumber.has(Number(match[1]))) return null;
    byNumber.set(Number(match[1]), q);
  }
  const used = new Set();
    ...
    function gap({ number, question: q }) {
      ...
      return jsxs('span', { className: `ri-inline-gap ${status}`, 'data-question-id': q.id, children: [
        ...
        q.wordBank && jsxs('select', {
          ...
          children: [jsx('option', { value: '', children: 'Choose a word…' }), ...q.wordBank.map(word => jsx('option', { value: word, children: word }, word))]
        }),
        (!q.wordBank || q.requiresForm) && jsx('input', {

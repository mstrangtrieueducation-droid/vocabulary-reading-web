...
function getUnitNumber() {
  ...
  const match = window.location.pathname.match(/ri2-u(\d{2})/i);
...
function makeSubmitUrl(unit) {
  ...
  const url = new URL("https://docs.google.com/forms/d/e/1FAIpQLSfq_3Cv80fXEojG0TvwsxHReh3zUieo8lr5SoEBTkb3qL2Vhw/viewform");
...
function hydrate(unit) {
  ...
  document.querySelector("#unit-summary").textContent =
    `${unit.entries.length} mục từ đọc hiểu học thuật: học nghĩa, cách dùng và phát âm chuẩn.`;
...
const unit = window.RI2_UNITS?.[unitNumber];
if (!unit) {
